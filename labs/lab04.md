...
```
In the EC2 console copy the **Public IPv4 address** of `userN-web-1` and open `http://<public-ip>` in a new tab (http, not https). You should see the instance ID and `us-east-2a`.
...
3. **Launch instance**. When Running, browse to **its** public IP. Different instance ID, **`us-east-2b`** — and you never logged in.
